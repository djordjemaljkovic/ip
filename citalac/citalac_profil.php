<html>
    ...
    <body>
        ...
        <table>
            ...
            <?php
                $id = $_SESSION['citalac'];
                $result = mysqli_query($conn, "select korIme,lozinka,imePrezime,adresa,telefon,imejl,slika from citaoci where korIme='$id';");
                if(mysqli_num_rows($result)>0){
                    #session_start(); 
                    while($row = mysqli_fetch_assoc($result)){
                        #setcookie('citalac',$row['korIme']); 
            ?>
            <form name="mojaforma" action="citalac_profilfajl.php" method="POST" enctype="multipart/form-data">
                            <tr>
                                <td>
