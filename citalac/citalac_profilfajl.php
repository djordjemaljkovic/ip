...
if(isset($_POST['profil'])){
    ...
    if(count($errors) == 0) {
        ...
        if(!empty($imePrezime)){
        $query1 = mysqli_query($conn, "UPDATE citaoci SET imePrezime = '$imePrezime' WHERE korIme='$idk'");
        }
        if(!empty($adresa)){
        $query2 = mysqli_query($conn, "UPDATE citaoci SET adresa = '$adresa' WHERE korIme='$idk'");
        }
        if(!empty($telefon)){
        $query3 = mysqli_query($conn, "UPDATE citaoci SET telefon = '$telefon' WHERE korIme='$idk'");
        }
        if(!empty($imejl)){
        $query4 = mysqli_query($conn, "UPDATE citaoci SET imejl = '$imejl' WHERE korIme='$idk'");
        }
        if(isset($_FILES['slikaProfil']) && !empty($_FILES['slikaProfil']) && $data!=""){
        $query5 = mysqli_query($conn, "UPDATE citaoci SET slika = '$data' WHERE korIme='$idk'");
        }
        $query6 = mysqli_query($conn, "SELECT lozinka FROM citaoci WHERE korIme='$idk'");
        $row = mysqli_fetch_assoc($query6);
        if($stara != ""){
        if($row['lozinka']== $stara && !empty($nova)){
        $nova = md5($nova);
        $query7 = mysqli_query($conn, "UPDATE citaoci SET lozinka = '$nova' WHERE korIme='$idk'");
        }}
        ...
        ?>
    <a href="citalac_profil.php">Nazad</a>
    <?php
    }
}
...
<html>
    ...
    <body>
        <div  class="sve">
        <?php include_once("citalac_meni.php"); ?>
